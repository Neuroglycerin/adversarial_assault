...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session() as sess:
      ...
      saver0.restore(sess, 'inception_v3.ckpt')

      ai3_vars = {('InceptionV3' + v.op.name.lstrip('AdvInceptionV3')): v
        for v in tf.get_collection(tf.GraphKeys.VARIABLES, scope='AdvInceptionV3')}
      saver1 = tf.train.Saver(var_list=ai3_vars)
      saver1.restore(sess, 'adv_inception_v3.ckpt')

      saver2 = tf.train.Saver(tf.get_collection(
        tf.GraphKeys.GLOBAL_VARIABLES, scope='InceptionResnetV2'))
      saver2.restore(sess, 'ens_adv_inception_resnet_v2.ckpt')
