...
tf.flags.DEFINE_float(
    'p_threshold_antitarget', 0.63, 'What split of probabilities to anti-target.')
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        logits = tf.reduce_mean(tf.stack(all_logits, axis=2), axis=2)
        ...
        vec_true = tf.ones([FLAGS.batch_size, 1], dtype=tf.bool)
        ...
        index_next_highest = tf.argmax(tf.cast(class_is_next_highest, tf.uint8),
                                       axis=-1,
