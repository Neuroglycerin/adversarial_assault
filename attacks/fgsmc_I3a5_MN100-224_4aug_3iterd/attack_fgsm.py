...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        for iter_count in range(1, FLAGS.max_iter):
            ...
            should_run_update = tf.cond(
                should_run_update,
                lambda: tf.logical_and(tf.less(iter_count, iter_limit),
                                       test_accuracy(logits_stack)),
                lambda: should_run_update)

            # Maybe update x_adv
            logits = tf.cond(
                should_run_update,
                lambda: tf.reduce_sum(logits_stack, axis=0, keep_dims=True),
                lambda: prev_logits)
