...
def augment_single_pre_resize(image):
    ...
    rotation_max_angle = 5
    brightness_max_delta = 0.06 * 2
    contrast_max_ratio = 1.1
    saturation_max_ratio = 1.1
    hue_max_degree = 15
...
def augment_batch_pre_resize(x):
    ...
    for image in images:
        for i in range(FLAGS.num_aug):
            augmented_images.append(augment_single_pre_resize(image))
    return tf.stack(augmented_images, axis=0)
