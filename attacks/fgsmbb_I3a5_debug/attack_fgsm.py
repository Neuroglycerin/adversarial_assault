...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        def update_logits(x):
            ...
            for model in model_stack.models:
                ...
                if FLAGS.num_aug > 1:
                    ...
                    model_logits = tf.reduce_mean(model_logits, axis=0, keep_dims=True)
        ...
        def update_x(x, local_logits):
            ...
            mu, var = tf.nn.moments(grad, range(len(grad.shape)))
            std = tf.sqrt(var)
            average_grad = tf.reduce_mean(tf.abs(grad))
            scaled_signed_grad = eps * tf.sign(grad)
            x_next = tf.stop_gradient(x + scaled_signed_grad)
            x_next = tf.clip_by_value(x_next, x_min, x_max)
            return x_next, mu, std

        # We definitely update here
        x_adv, mu, std = update_x(x_input, logits)
        ...
        for iter_count in range(1, FLAGS.max_iter):
            ...
            # stopped computing, otherwise x_adv has no return value.
            ...
            should_run_update = tf.logical_and(have_time_for_one_more,
                                               label_is_right)
            # Maybe update x_adv
            x_adv, _, __ = update_x(x_adv, logits)
        ...
        with tf.Session() as sess:
            ...
            for filenames, images in load_images(FLAGS.input_dir, batch_shape):
                ...
                adv_images, batch_num_iter, batch_mu, batch_std = sess.run(
                    [x_adv, num_iter_used, mu, std],
                    feed_dict={x_input: images, iter_limit: local_iter_limit})
                save_images(adv_images, filenames, FLAGS.output_dir)

                num_iter_tally.append(batch_num_iter)
                mu_tally.append(mu)
                std_tally.append(std)
                ...
                if num_samples_shown // 100 > last_report_nsamp // 100:
                    ...
                    logging.info('After {} samples: Took {} sec/sample. '
                                 ...
                                 'mu = {}+/-{}, std: {}+/-{}'.format(
                                    ...
                                    np.mean(tally),
                                    np.std(tally),
                                    np.mean(np.array(mu_tally)),
                                    np.std(np.array(mu_tally)),
                                    np.mean(np.array(std_tally)),
                                    np.std(np.array(std_tally)),
                                    )
                                 )
