    ...
    with tf.Graph().as_default():
        ...
        def noisify(x):
            ...
            noise = tf.random_uniform(
                shape=tf.shape(x), minval=-dist, maxval=dist, dtype=tf.float32)
            return x + noise


        with slim.arg_scope(inception.inception_v3_arg_scope()):
            logits, end_points = inception.inception_v3(
                noisify(x_adv), num_classes=num_classes, is_training=False, reuse=True)
        all_logits += logits
        all_logits += end_points['AuxLogits']
        ...
        with slim.arg_scope(mobilenet_v1.mobilenet_v1_arg_scope()):
            logits, end_points = mobilenet_v1.mobilenet_v1(
                noisify(x_224), num_classes=num_classes, is_training=False, reuse=True,
                spatial_squeeze=False, scope='MobilenetV1_100_244')
        ...
        with slim.arg_scope(mobilenet_v1.mobilenet_v1_arg_scope()):
            logits, end_points = mobilenet_v1.mobilenet_v1(
                noisify(x_192), num_classes=num_classes, is_training=False, reuse=True,
                spatial_squeeze=False, scope='MobilenetV1_100_192')
        logits = tf.squeeze(logits, axis=(1, 2))
        preds = tf.squeeze(end_points['Predictions'], axis=(1, 2))
        all_logits += logits
        ...
        with slim.arg_scope(mobilenet_v1.mobilenet_v1_arg_scope()):
            logits, end_points = mobilenet_v1.mobilenet_v1(
                noisify(x_160), num_classes=num_classes, is_training=False, reuse=True,
                spatial_squeeze=False, scope='MobilenetV1_100_160')
        logits = tf.squeeze(logits, axis=(1, 2))
        preds = tf.squeeze(end_points['Predictions'], axis=(1, 2))
        all_logits += logits
        ...
        with slim.arg_scope(mobilenet_v1.mobilenet_v1_arg_scope()):
            logits, end_points = mobilenet_v1.mobilenet_v1(
                noisify(x_128), num_classes=num_classes, is_training=False, reuse=True,
                spatial_squeeze=False, scope='MobilenetV1_100_128')
        logits = tf.squeeze(logits, axis=(1, 2))
        preds = tf.squeeze(end_points['Predictions'], axis=(1, 2))
        all_logits += logits
        ...
        with tf.Session() as sess:
            ...
            var_dict = {('InceptionV3' + v.op.name.lstrip('AdvInceptionV3')): v
